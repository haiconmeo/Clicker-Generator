...
export function buildClicker(
  ...
  if (decoration) {
    try {
      const dbb = decoration.boundingBox();
      const dx = dbb.max[0] - dbb.min[0];
      const dy = dbb.max[1] - dbb.min[1];
      const dz = dbb.max[2] - dbb.min[2];
      const plateW = plateBB.max[0] - plateBB.min[0];
      const plateH = plateBB.max[1] - plateBB.min[1];
      const maxDecorXY = Math.max(4, Math.min(plateW, plateH) * 0.55);
      const maxDecorZ = 6;
      if (dx > 0.01 && dy > 0.01 && dz > 0.01) {
        const userScale = Math.max(0.05, Math.min(4, params.decorationScale ?? 1));
        const scale = Math.min(maxDecorXY / Math.max(dx, dy), maxDecorZ / dz) * userScale;
        const cx = (dbb.min[0] + dbb.max[0]) / 2;
        const cy = (dbb.min[1] + dbb.max[1]) / 2;
        const raised = track(
          decoration
            .translate([-cx, -cy, -dbb.min[2]])
            .scale([scale, scale, scale])
            .translate([0, 0, slabTopZ - 0.04]),
        );
        if (!raised.isEmpty()) {
          parts.push(toPart(raised, 'cap', 'top', params.decorationColorRgb ?? params.baseFilamentRgb, 'top-decoration-3d'));
        }
      }
    } catch {
      warnings.push('Could not place the 3D decoration on the cap.');
    }
  }
