...
let currentDecoration3mf: ArrayBuffer | null = null;
...
const ui = createUi(sidebarLeft, sidebarRight, statusEl, {
  ...
  onImportMode: (mode) => {
    ...
    if (mode === 'model') {
      regionSet ??= createModelBaseRegionSet();
      store.set({ palette: [], status: 'Upload a 3MF decoration to build a clicker.' });
      return;
    }
    reprocess();
  },
  ...
  onModelUpload: async (file) => {
    try {
      store.set({ building: true, status: 'Reading 3D decoration…' });
      currentDecoration3mf = await file.arrayBuffer();
      regionSet ??= createModelBaseRegionSet();
      document.getElementById('modelDecorationStatus')!.textContent = `Selected: ${file.name}`;
      store.set({ building: true, status: `Building clicker with 3D decoration: ${file.name}` });
      rebuild();
    } catch (err) {
      store.set({ building: false, status: 'Error reading 3D decoration: ' + String(err) });
    }
  },
  onModelClear: () => {
    currentDecoration3mf = null;
    document.getElementById('modelDecorationStatus')!.textContent = 'No 3D decoration selected.';
    store.set({ status: '3D decoration removed.' });
    debouncedRebuild();
  },
  onModelScale: (scale) => {
    store.set({ modelScale: scale });
    debouncedRebuild();
  },
...
const HISTORY_FIELDS = [
  ...
  'imageDepth', 'tolerance', 'stemTolerance', 'switches', 'keychain', 'modelScale',
] as const;
let history: string[] = [];
let histIndex = -1;
...
worker.onmessage = (e: MessageEvent<GeometryResponse>) => {
  ...
  switch (msg.type) {
    ...
    case 'initDone':
      ...
      store.set({
        status: 'Ready. Import an image, SVG, icon, text, or 3MF.',
      });
...
function reprocess() {
  ...
  } else if (s.importMode === 'model') {
    regionSet = regionSet ?? createModelBaseRegionSet();
  }
  ...
  if (palette.length === 0 && !currentDecoration3mf) {
...
function rebuild(quiet = false) {
  if (!regionSet) return;
  if (regionSet.regions.length === 0 && !currentDecoration3mf) return;
  if (!assetsReady) {
  ...
  const params: BuildParams = {
    ...
    decorationColorRgb: [230, 230, 230],
    decorationScale: s.modelScale,
  ...
  const decoration3mf = currentDecoration3mf ? currentDecoration3mf.slice(0) : null;
  worker.postMessage({ type: 'buildClicker', regions, outline: regionSet.outline, params, decoration3mf }, decoration3mf ? [decoration3mf] : []);
}
