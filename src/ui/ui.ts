...
export interface UiState {
  ...
  view: ViewMode;
  showSwitch: boolean;
  importMode: 'image' | 'svg' | 'icon' | 'text' | 'model';
  modelScale: number;
...
export interface UiCallbacks {
  ...
  onImportMode(mode: 'image' | 'svg' | 'icon' | 'text' | 'model'): void;
...
export function createUi(
  ...
  sidebarRight.innerHTML = `
    <div class="section legend-section">
      ...
      <div class="import-grid" id="importTabs" role="tablist">
        ...
        <button class="import-card" data-mode="model" type="button">
          <span class="card-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/>
          ...
          <span class="card-label">3MF</span>
        </button>
      </div>
      ...
      <!-- 3MF Panel -->
      <div id="modelPanel" class="mode-panel" hidden>
        <p class="hint-text">
          Upload a 3MF model to place it as a raised decoration on top of the current clicker.
        </p>
        <label class="upload-cta">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><path d="M3.3 7 12 12l8.7-5"/><path d="M12 22V12"/></svg>
          Upload 3MF decoration
          <input id="modelUpload" type="file" accept=".3mf,model/3mf" />
        </label>
        <div class="hint-text" id="modelDecorationStatus">No 3D decoration selected.</div>
        <div class="field" style="margin-top: 10px;">
          <label for="modelScale">3MF size</label>
          <div class="slider-row">
            <input type="range" id="modelScale" min="0.25" max="2" step="0.05" />
            <input type="text" class="val" id="modelScaleVal" />
          </div>
        </div>
        <button class="secondary" id="clearModelDecoration" type="button" style="width:100%; margin-top:8px;">Remove 3D decoration</button>
      </div>
    </div>
  ...
  const modelUpload = $<HTMLInputElement>('modelUpload');
  modelUpload.addEventListener('change', () => {
    const f = modelUpload.files?.[0];
    if (f) cb.onModelUpload(f);
    modelUpload.value = '';
  });
  $('clearModelDecoration').addEventListener('click', () => cb.onModelClear());
  ...
  bindValInput('modelScaleVal', $<HTMLInputElement>('modelScale'), cb.onModelScale, (v) => v / 100);
  $<HTMLInputElement>('modelScale').addEventListener('input', (e) =>
    cb.onModelScale(parseFloat((e.target as HTMLInputElement).value)),
  );
  ...
  const TUTORIAL_STEPS: TutorialStep[] = [
    {
      focus: 'right',
      target: '#importTabs',
      title: 'Nguồn thiết kế',
      text: 'Chọn cách tạo mẫu clicker 3D. Bạn có thể tải lên <strong>ảnh</strong> riêng (PNG nền trong suốt là tốt nhất), chọn từ <strong>1700+ icon vector</strong>, nhập file <strong>SVG</strong>, hoặc nhập <strong>chữ</strong> tùy chỉnh.',
      arrow: 'right'
    },
    {
      focus: 'right',
      target: '#export',
      title: 'Xuất file 3MF',
      text: 'Khi đã hài lòng với thiết kế, bấm vào đây để tải file <strong>3MF</strong> sẵn sàng in. Định dạng 3MF giữ dữ liệu nhiều màu và có thể mở trực tiếp trong slicer như Bambu Studio, OrcaSlicer hoặc PrusaSlicer.',
      arrow: 'right'
    },
    {
      focus: 'right',
      ...
      title: 'Cài đặt dự án',
      text: 'Lưu thiết kế đang làm dưới dạng file <code>.json</code> để sửa tiếp sau này, tải lại dự án cũ, đổi giao diện sáng/tối, hoặc mở lại hướng dẫn này.',
      arrow: 'right'
    },
    {
      focus: 'center',
      target: '#app',
      title: 'Khu vực xem trước 3D',
      text: 'Đây là nơi xem trước thiết kế ở dạng 3D. Mẹo: <strong>giữ chuột trái và kéo</strong> để xoay, <strong>giữ chuột phải và kéo</strong> để di chuyển khung nhìn, và <strong>cuộn chuột</strong> để phóng to/thu nhỏ.',
      arrow: 'none',
      cardPosition: 'left'
    },
    {
      focus: 'center',
      target: '#editModeBar',
      title: 'Chế độ màu và chiều cao',
      text: 'Chuyển giữa <strong>Color Mode</strong> để tô từng vùng bằng màu filament khác nhau và <strong>Extrude Mode</strong> để nâng một số phần của thiết kế, chỉnh độ dày/độ cao và vát cạnh phần nổi. Nếu dùng <strong>Text</strong>, bật <strong>Separate letters</strong> để tô màu hoặc đùn từng chữ riêng.',
      arrow: 'up',
      cardPosition: 'left'
    },
    {
      ...
      title: 'Xem trước lắp ráp',
      text: 'Xem mẫu ở trạng thái <strong>Assembled</strong> hoặc chuyển sang <strong>Exploded</strong> để thấy các phần in 3D khớp với nhau như thế nào. Bạn cũng có thể hiện switch MX tham chiếu để kiểm tra độ vừa.',
      arrow: 'left'
    },
    {
      focus: 'left',
      ...
      title: 'Hình dạng đế',
      text: 'Chọn hình dạng tổng thể cho đế clicker. Có thể dùng <strong>Custom Outline</strong> để bám theo viền hình đã nhập, hoặc các dạng chuẩn như <strong>Circle</strong> và <strong>Hexagon</strong>. Kích thước tổng thể cũng được chỉnh ở đây.',
      arrow: 'left'
    },
    {
      focus: 'left',
      target: '#geometrySettingsContainer',
      title: 'Cài đặt hình học và kiểu dáng',
      text: 'Tinh chỉnh mẫu trong các mục có thể mở/đóng. <strong>1 · Colors &amp; Smoothing</strong> dùng để chọn màu filament và làm mượt viền. <strong>2 · More Settings</strong> thêm móc keychain, đổi độ dày và chỉnh <strong>dung sai socket/stem của switch</strong> để kiểm soát độ khít giữa phần trên và phần dưới.',
      arrow: 'left'
    },
    {
      focus: 'left',
      target: '#sectionSwitch',
      title: 'Đặt vị trí switch',
      text: 'Mở mục <strong>3 · Switch</strong> để <strong>di chuyển và xoay</strong> switch MX. Hữu ích khi switch không nằm đẹp ở chính giữa thiết kế.',
      arrow: 'left'
    },
    {
      focus: 'left',
      target: '#historyControls',
      title: 'Hoàn tác, làm lại và làm mới',
      text: 'Dùng các nút này để hoàn tác hoặc làm lại các bước thiết kế, hoặc làm mới mẫu về trạng thái ban đầu.',
      arrow: 'left'
    }
  ];
      ...
      card.innerHTML = `
        ...
        <div class="tutorial-controls">
          ...
          <div class="tutorial-nav">
            <button class="secondary" id="tutPrev" ${stepIndex === 0 ? 'disabled' : ''}>Trước</button>
            <button class="primary" id="tutNext">${stepIndex === TUTORIAL_STEPS.length - 1 ? 'Xong' : 'Tiếp'}</button>
          </div>
        </div>
      `;
  ...
  function showTutorialPrompt() {
    ...
    wm.innerHTML = `
      <div class="welcome-card" style="align-items: center; text-align: center; width: 380px; padding: 32px;">
        <h2 style="margin-bottom: 8px;">Hướng dẫn sử dụng</h2>
        <p style="margin-bottom: 24px;">Bạn có muốn xem hướng dẫn tương tác không?</p>
        <div style="display: flex; gap: 12px; justify-content: center; width: 100%;">
          <button class="secondary" id="tutPromptNo" style="flex: 1;">Không</button>
          <button class="primary" id="tutPromptYes" style="flex: 1;">Có</button>
        </div>
      </div>
    `;
